...
void* handle_client(void* arg) {
    ...
    //... ваш код приема и обработки данных
    char buffer[1024];
    ...
    if (bytes_received < 0) {
        ...
        if (arg_count < 3) { // Проверяем на корректное количество аргументов (должно быть 4: --file, --query, op1, op2)
        ...
        if (!fileToken || !queryToken || !op1Token) {
